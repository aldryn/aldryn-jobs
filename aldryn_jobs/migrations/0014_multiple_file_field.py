...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        old_attachment_fields = ['attachment', 'attachment_2', 'attachment_3', 'attachment_4']
        for application in orm.JobApplication.objects.filter(
            Q(attachment__isnull=False) |
            Q(attachment_2__isnull=False) |
            Q(attachment_3__isnull=False) |
            Q(attachment_4__isnull=False)
        ):
            for field in old_attachment_fields:
                att_file = getattr(application, field, None)
                if att_file:
                    att = orm.JobApplicationAttachment(
